...
public class CoatingSampleProblemActivity extends BaseActivity {

    @BindView(R.id.rv_coating_sample_problem)
    RecyclerView rvCoatingSampleProblem;
    /**
     * 多张图片相关
     */
    ...
    protected int getLayoutId() {
        return R.layout.activity_coating_sample_problem;
    }
    ...
    protected void initData() {
        ...
        baseTitle.setText("竞品涂料样板效果及问题");
        myImgAdapter = new MyImgAdapter(CoatingSampleProblemActivity.this, mImageList);
        gridLayoutManager = new GridLayoutManager(CoatingSampleProblemActivity.this, 3);
        rvCoatingSampleProblem.setLayoutManager(gridLayoutManager);
        rvCoatingSampleProblem.setHasFixedSize(true);
        rvCoatingSampleProblem.setItemAnimator(new DefaultItemAnimator());
        rvCoatingSampleProblem.setAdapter(myImgAdapter);
        myImgAdapter.setOnItemClickListener(new MyImgAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, final int position) {

                RxPermissions rxPermissions = new RxPermissions(CoatingSampleProblemActivity.this);
                rxPermissions.request(Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE).subscribe(new Consumer<Boolean>() {
                    ...
                    public void accept(Boolean aBoolean) throws Exception {
                        if (aBoolean) {
                            Intent picIntent = new Intent(CoatingSampleProblemActivity.this, ImageActivity.class);
            ...
            public void accept(Boolean aBoolean) throws Exception {
                if (aBoolean) {
                    MultiImageSelector.create()
                            ...
                            .multi() // 多选模式, 默认模式;
                            .origin(mImageList)
                            .start(CoatingSampleProblemActivity.this, MULTI_IMG);
                } else {
                    // 用户拒绝了该权限，并且选中不再询问,引导用户跳转权限管理页面
                }
            }
